...
async function startServer() {
  try {
    ...
    const redirectAllHttpToHttps = config.get<boolean>(
      "server.https.redirectAllHttpToHttps"
    );
    ...
    if (httpEnabled) {
      if (httpsEnabled && redirectAllHttpToHttps) {
        ...
        redirectApp.listen(httpPort, () => {
          logger.info(
            `HTTP redirect running on port ${httpPort} to HTTPS ${httpsPort}`
          );
        });
      } else {
        app.listen(httpPort, () => {
    ...
    if (httpsEnabled) {
      const sslKeyPath =
        process.env.SSL_KEY_PATH || config.get<string>("server.https.keyPath");
      const sslCertPath =
        process.env.SSL_CERT_PATH ||
        config.get<string>("server.https.certPath");

      if (!fs.existsSync(sslKeyPath) || !fs.existsSync(sslCertPath)) {
        logger.warn(
          "SSL key or certificate file not found. HTTPS server not started."
        );
      } else {
        const httpsOptions = {
