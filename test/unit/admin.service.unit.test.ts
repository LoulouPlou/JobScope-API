...
describe('adminService', () => {
  ...
  test('infoUser , USER_NOT_FOUND', async () => {
    ...
    await expect(adminService.infoUser('x')).rejects.toMatchObject({ code: 'USER_NOT_FOUND' });
  });
  ...
  test('updateUser ,PASSWORD_UPDATE_FORBIDDEN', async () => {
    await expect(adminService.updateUser('1', { password: '123' } as any)).rejects.toMatchObject({
  ...
  test('updateUser , USER_NOT_FOUND', async () => {
    ...
    await expect(adminService.updateUser('x', { firstName: 'Test' })).rejects.toMatchObject({
      code: 'USER_NOT_FOUND',
    });
  });
  ...
  test('deleteUser ,USER_NOT_FOUND', async () => {
    ...
    await expect(adminService.deleteUser('x')).rejects.toMatchObject({ code: 'USER_NOT_FOUND' });
  });
});
